...
$requestEmail = $_POST['search_email'];
}

try {
    $user = $auth->getUserByEmail($requestEmail);
    $uid = $user->uid;
    $email = $user->email;
...
// $fetchedData = $database->getReference('users/'.$uid)->getValue();
				...
				<div class="col-md-12">
					...
					<div class="card hold">
						<div class="card-header hold">
							<h3>
								 Fetched user information: {1 result found}
                                 <?php
						...
						<div class="mb-3 ">
							...
							<table class=" table table-bordered table-striped bg-dark text-white">
								...
								?>
											...
											<td>
												<a href="claimsedit.php?id=<?=$uid;?>" class="btn btn-primary btn-sm">Edit</a>
											</td>
											
											<td>
												<form method="post" action="deleteDb.php">
													<button type="submit" name="userDelete" value="<?=$uid;?>" class="btn btn-danger btn-sm">DeleteData</button>
												</form>
											</td>
											</tr>
								<?php
